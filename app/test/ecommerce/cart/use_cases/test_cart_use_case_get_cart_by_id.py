...
from app.ecommerce.cart.use_cases.get_cart_by_id_use_Case import get_cart_by_id_use_case
...
class TestCartUseCaseGetCartById(TestCase):
    def setUp(self):
    ...
    def test_get_cart_by_id_with_invalid_request(self):
    ...
    def test_get_cart_by_id_with_valid_request(self):
        with current_app.test_request_context("/?user_email={}".format(self.user["email"])):
            req = validate_params_id(self.cart["id"])
            res = get_cart_by_id_use_case(req)
